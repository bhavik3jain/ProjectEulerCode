...
public class Euler17 {
	...
	private static String process(String triple) {
		String response = "";
		if (triple.length() == 1) { triple = "00" + triple; }
 		else if (triple.length() == 2) { triple = "0" + triple; }
		if (triple.equals ("000")) { return response; }
		
		int val = Integer.valueOf(triple);
		if (val >=1 && val <= 9) { return actualWords[val]; }
		if (val >=11 && val <= 19) { return special[val]; }
		...
		if (val % 10 == 0 && val >= 10) {
			if (space) { response += " "; space = false; }
			response += tens[val/10];
			return response;
		}
		
		if (val == 0) { return response; }
		if (space) { response += " "; space = false; }
		if (val >= 11 && val <= 19) {
		...
		if (val < 20) { addHyphen = false; }
		val = val % 10;
		if (val != 0 && addHyphen) { response += ""; }
